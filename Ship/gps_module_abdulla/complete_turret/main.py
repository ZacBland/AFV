...
def homing_turret_menu():
    print("Homing Turret...")
    ip = input("Enter the IP address of the FLIR camera: ")

    while True:  # Start of the continuous loop
        # Add an option for the user to exit the loop
        user_input = input("Press Enter to start homing or type 'exit' to stop: ")
        if user_input.lower() == 'exit':
            print("Exiting homing turret loop...")
            break
...
def send_command_to_pi(device_type, command1, command2=None):
    ...
    ip_table = {
        ...
        "nozzle": "169.254.47.131",
        "relay": "169.254.47.131",
        "motor": "169.254.47.131",
        "turret": "169.254.47.131"
    ...
    ssh_command = ["ssh", f"pi@{device_ip}", "python3", f"/home/pi/Desktop/{script_path}"]
