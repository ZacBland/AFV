...
def main():
    ...
    polygon_coords = [
        [-97.07643951347823, 36.15077249982036],  # Bottom Left
        [-97.07643951347823, 36.17363997553162],  # Top Left
        [-97.09497894218917, 36.17363997553162],  # Top Right
        [-97.09497894218917, 36.15077249982036]   # Bottom Right
    ]

    # Calculate center coordinates
    center_coords = [
        (polygon_coords[0][0] + polygon_coords[2][0]) / 2,
        (polygon_coords[0][1] + polygon_coords[2][1]) / 2
    ]

    # Convert to string and pass to the existing code
    coords_in = str(center_coords[1]) + ',' + str(center_coords[0])
    ...
    min_longitude, min_latitude, max_longitude, max_latitude = bounding_polygon.bounds
    #bounding polygon coordinates to pixel coordinates
    bounding_polygon_pixels = convert_coords_to_pixels(bounding_polygon, img, min_longitude, max_longitude, min_latitude, max_latitude)

    # Create an empty mask
    mask = np.zeros_like(img)

    # Fill the area inside the bounding polygon with white
    cv2.fillPoly(mask, [np.array(bounding_polygon_pixels)], (255,255,255))

    # Apply the mask to the image
    img = cv2.bitwise_and(img, img, mask=mask[:,:,0])

    cv2.imwrite('Z_FENCED_2.jpg', img)
    import piexif
    from piexif import helper
    # Load the image
    img = cv2.imread('Z_FENCED_2.jpg')

    # Save the image temporarily
    cv2.imwrite('temp.jpg', img)

    # Load the EXIF data from the image
    exif_dict = piexif.load('temp.jpg')

    # Define the GPS data
    gps_ifd = {
        piexif.GPSIFD.GPSLatitudeRef: 'N',
        piexif.GPSIFD.GPSLatitude: helper.degToRationalTuple(min_latitude),
        piexif.GPSIFD.GPSLongitudeRef: 'W',
        piexif.GPSIFD.GPSLongitude: helper.degToRationalTuple(min_longitude),
    }

    # Embed the GPS data into the EXIF data
    ...
    piexif.insert(exif_bytes, 'temp.jpg')

    # Rename the image
    os.rename('temp.jpg', 'Z_FENCED_2.jpg')
...
# ----- QUADRENT FUNCTIONS ----------------------------------------------------
# =============================================================================
def convert_coords_to_pixels(polygon, img, min_longitude, max_longitude, min_latitude, max_latitude):
    # Define the resolution (pixels per degree)
    resolution_x = img.shape[1] / (max_longitude - min_longitude)
    resolution_y = img.shape[0] / (max_latitude - min_latitude)

    # Define the coordinates of the top-left corner of the image
    top_left_longitude = min_longitude
    top_left_latitude = max_latitude

    # Convert the polygon coordinates to pixel coordinates
    polygon_pixels = []
    for point in polygon.exterior.coords:
        pixel_x = (point[0] - top_left_longitude) * resolution_x
        pixel_y = (top_left_latitude - point[1]) * resolution_y
        polygon_pixels.append((int(pixel_x), int(pixel_y)))
